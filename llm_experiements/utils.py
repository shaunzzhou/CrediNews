...
def save_results(
    ...
    perf_original_model_val_data_rewritten = pd.DataFrame(perf_original_model_val_data_rewritten, index=[0])
    perf_combined_model_val_data = pd.DataFrame(perf_combined_model_val_data, index=[0])
    perf_combined_model_val_data_rewritten = pd.DataFrame(perf_combined_model_val_data_rewritten, index=[0])
    perf_rewritten_model_val_data = pd.DataFrame(perf_rewritten_model_val_data, index=[0])
    perf_rewritten_model_val_data_rewritten = pd.DataFrame(perf_rewritten_model_val_data_rewritten, index=[0])  
    df = pd.concat([
        perf_original_model_val_data, 
        perf_original_model_val_data_rewritten,
        perf_combined_model_val_data,
    ...
    df["model"] = model
    df["train-data"] = ["original", "original", "combined", "combined", "rewritten", "rewritten"]
    df["test-data"] = ["original", "rewritten", "original", "rewritten", "original", "rewritten"]
    
    df2 = df[["model", "train-data", "test-data", "accuracy", "precision", "recall", "f1_score"]].reset_index(drop=True)
...
def plot_results(
    ...
    def plot_subplot(ax, data_left, data_right, title, left_color, right_color):
        ...
        bars_left = ax.bar(x - bar_width/2, list(data_left.values()), 
                           width=bar_width, color=left_color)
        ax.set_ylim(0, ylim)
        ax.set_xticks(x)
        ax.set_xticklabels(categories)
        for i, val in enumerate(data_left.values()):
            ax.text(x[i] - bar_width/2, val + 0.01, f"{val*100:.2f}%", 
        ...
        ax2 = ax.twinx()
        bars_right = ax2.bar(x + bar_width/2, list(data_right.values()), 
                             width=bar_width, color=right_color)
        ax2.set_ylim(0, ylim)
        for i, val in enumerate(data_right.values()):
            ax2.text(x[i] + bar_width/2, val + 0.01, f"{val*100:.2f}%", 
                     ha='center', va='bottom', fontsize=8)
        
        # Add right legend at top right for LLM-rewritten Test Data
        ax2.legend([bars_right], ["LLM-rewritten Test Data"], loc="upper right", fontsize=8)
